...
Implementa as 3 sugestões do orientador para elevar o trabalho:
  ...
  SUGESTÃO 3: Predição Conformal Adaptativa (ACI)
    - Implementa Adaptive Conformal Inference (Gibbs & Candès, 2021)
    - Mantém garantias de cobertura mesmo com distribution shift
    - Aplica-se a RF, XGBoost, SARIMAX, LSTM, Bayesianos
...
- Zaffran, M., Féron, O., Goude, Y., Josse, J., & Dieuleveut, A. (2022).
  Adaptive conformal prediction for time series. ICML.
...
def executar_predicao_conformal(df, metodo='aci', gamma=0.02, janela=30):
    """
    Predição Conformal adaptativa com ACI (Gibbs & Candès, 2021).
    Mantém garantias de cobertura mesmo sob distribution shift.

    Parâmetros:
    - metodo: 'split' (fixo) ou 'aci' (adaptativo)
    - gamma: taxa de atualização do quantil (0.01 a 0.05) - padrão 0.02
    - janela: número de resíduos recentes considerados para o quantil (padrão 30)
    ...
    """
    print("\n" + "="*60)
    print("  SUGESTÃO 3: PREDIÇÃO CONFORMAL ADAPTATIVA (ACI)")
    print(f"  Modo: {metodo.upper()} | gamma={gamma} | janela={janela}")
    print("="*60)

    target = config.TARGET_VAR

    # Preparar dados (mesmo código original)
    colunas_excluir = ['country_code', 'year', target, 'pais', 'country_name']
    feature_cols = [c for c in df.columns if c not in colunas_excluir and df[c].dtype in ['float64', 'int64', 'float32']]
    ...
    # Split temporal (mesmo do original)
    ...
    # Carregar modelos (mesmo do original)
    modelos = {}
    ...
    for nome in nomes_modelos:
        ...
        if model is not None:
            ...
            if hasattr(model, 'n_features_in_') and model.n_features_in_ == X_train.shape[1]:
                ...
                # Modelos como SARIMAX/LSTM podem não ter n_features_in_
                modelos[nome] = model
    ...
    print(f"  Modelos compatíveis: {list(modelos.keys())}")
    ...
    for nome_modelo, model in modelos.items():
        print(f"\n  --- {nome_modelo} (modo {metodo.upper()}) ---")

        # Previsões na calibração e teste
        try:
        ...
        # Resíduos iniciais da calibração
        ...
        for alpha in alphas:
            ...
            if metodo == 'split':
                # ---- CONFORMAL FIXO (ORIGINAL) ----
                quantil_level = np.ceil((1 - alpha) * (n_cal + 1)) / n_cal
                q_hat = np.quantile(residuos_cal, min(quantil_level, 1.0))
                ...
                if alpha == 0.10:
                    for i in range(len(y_test)):
                        resultados_intervalos.append({
                            'Modelo': nome_modelo,
                            'Pais': test_paises[i] if i < len(test_paises) else '',
                            'Pais_Nome': config.PAISES.get(test_paises[i] if i < len(test_paises) else '', ''),
                            'Ano': int(test_anos[i]) if i < len(test_anos) else 0,
                ...
                # ---- ACI ADAPTATIVO ----
                # Inicializar com o quantil da calibração
                q_hat = np.quantile(residuos_cal, 1 - alpha)
                # Lista de resíduos que será atualizada online (janela móvel)
                residuos_online = list(residuos_cal)
                # Listas para guardar intervalos e cobertura
                lower_list = []
                upper_list = []
                coberturas = []
                ...
                for i in range(len(y_test)):
                    ...
                    # Gerar intervalo com q_hat atual
                    lower_i = y_pred_i - q_hat
                    upper_i = y_pred_i + q_hat
                    lower_list.append(lower_i)
                    upper_list.append(upper_i)
                    dentro_i = (y_true_i >= lower_i) and (y_true_i <= upper_i)
                    coberturas.append(dentro_i)

                    # Atualizar resíduo online (assim que o real é conhecido)
                    residuo_atual = abs(y_true_i - y_pred_i)
                    residuos_online.append(residuo_atual)
                    if len(residuos_online) > janela:
                        residuos_online.pop(0)

                    # Cobertura acumulada até o momento
                    cobertura_acumulada = np.mean(coberturas)

                    # Ajuste do nível de significância (alpha_t) conforme ACI
                    # Fórmula: alpha_t = alpha - gamma * (cobertura_acumulada - (1 - alpha))
                    alpha_t = alpha - gamma * (cobertura_acumulada - (1 - alpha))
                    # Limitar para evitar valores extremos
                    alpha_t = np.clip(alpha_t, 0.01, 0.99)

                    # Recalcular q_hat sobre a janela de resíduos recentes
                    q_hat = np.quantile(residuos_online, 1 - alpha_t)

                # Cobertura final
                cobertura_empirica = np.mean(coberturas) * 100
                largura_media = np.mean(np.array(upper_list) - np.array(lower_list))

                resultados_cobertura.append({
                    'Modelo': nome_modelo,
                    'Nivel_Confianca': f"{int(nivel_conf*100)}%",
                    'Alpha': alpha,
                    'Q_Hat': q_hat,  # último q_hat
                    'Cobertura_Empirica_Pct': cobertura_empirica,
                    'Cobertura_Nominal_Pct': nivel_conf * 100,
                    'Largura_Media_Intervalo': largura_media,
                    'N_Calibracao': n_cal,
                ...
                if alpha == 0.10:
                    for i in range(len(y_test)):
                        resultados_intervalos.append({
                            ...
                            'Y_Real': float(y_test[i]),
                            'Y_Pred': float(y_test_pred[i]),
                            'IC_Lower_90': float(lower_list[i]),
                            'IC_Upper_90': float(upper_list[i]),
                            'Largura_IC': float(upper_list[i] - lower_list[i]),
                            'Dentro_IC': bool(coberturas[i]),
                            'Residuo_Abs': float(abs(y_test[i] - y_test_pred[i])),
                        })

        # ----- Quartis via Bootstrap (usando a janela final de resíduos) -----
        if metodo == 'aci':
            residuos_finais = residuos_online  # última janela
        else:
            residuos_finais = residuos_cal

        n_bootstrap = 1000
        for i in range(len(y_test)):
            boot_residuos = np.random.choice(residuos_finais, size=n_bootstrap, replace=True)
    ...
    if len(df_cobertura) > 0:
        ...
        if len(df_cob_90) > 0:
            ...
            ax.set_title(f'Cobertura dos Intervalos Conformais (90%)\nModo: {metodo.upper()}', fontweight='bold')
            ax.legend()
            ax.set_ylim(0, 105)
            ...
            ax.grid(axis='y', alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(INOVACOES_DIR, 'conformal_cobertura_comparacao.png'), dpi=150, bbox_inches='tight')
        plt.close()
    ...
    if len(df_intervalos) > 0:
        ...
        if len(modelos_plot) > 0:
            ...
            plt.suptitle('Intervalos de Predição Conformal (90%) — Comparação entre Modelos',
                        fontsize=12, fontweight='bold', y=1.02)
            plt.tight_layout()
            plt.savefig(os.path.join(INOVACOES_DIR, 'conformal_intervalos_exemplo.png'), dpi=150, bbox_inches='tight')
            plt.close()
    ...
    if len(df_quartis) > 0:
        ...
        if len(df_q) > 0:
            ...
            ax.set_title(f'Fan Chart de Incerteza Preditiva — {config.PAISES.get(pais_ex, pais_ex)}\n'
                        f'(Modelo: {modelo_ex} | ACI Conformal com Bootstrap)',
                        fontsize=12, fontweight='bold')
            ax.legend(loc='best', fontsize=10)
            ax.grid(alpha=0.3)
            plt.tight_layout()
            plt.savefig(os.path.join(INOVACOES_DIR, 'conformal_fan_chart.png'), dpi=150, bbox_inches='tight')
            plt.close()

    # Gráfico 10: Incerteza por país
    if len(df_intervalos) > 0:
        fig, ax = plt.subplots(figsize=(12, 8))
        ...
        if len(df_larg) > 0:
            ...
            ax.set_title(f'Heterogeneidade da Incerteza Preditiva por País\n'
                        f'(Modelo: {modelo_1} | ACI Conformal)',
                        fontsize=12, fontweight='bold')
            ax.grid(axis='x', alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(INOVACOES_DIR, 'conformal_incerteza_por_pais.png'), dpi=150, bbox_inches='tight')
        plt.close()
...
def executar_passo10():
    ...
    3. Predição Conformal Adaptativa (ACI)
    ...
    # SUGESTÃO 3: PREDIÇÃO CONFORMAL ADAPTATIVA (ACI)
    # ============================================================
    print("\n  [3/3] Executando Sugestão 3: Predição Conformal Adaptativa (ACI)...")
    try:
        # Chamar a nova função com gamma=0.02 (padrão) e método ACI
        df_intervalos, df_cobertura, df_quartis = executar_predicao_conformal(
            df, metodo='aci', gamma=0.02, janela=30
        )
    ...
    metadados = {
        ...
        'descricao': 'Inovações metodológicas: Ruptures, Contrafactual SHAP, Predição Conformal Adaptativa (ACI)',
        ...
        'sugestao_3': {
            'nome': 'Predição Conformal Adaptativa (ACI)',
            ...
            'parametros': {'gamma': 0.02, 'janela': 30, 'metodo': 'ACI'},
        },
    }
    ...
    print(f"  Sugestão 3 (ACI Conformal):  {metadados['sugestao_3']['status']} — {len(df_intervalos)} intervalos")
    print(f"    Parâmetros ACI: gamma={metadados['sugestao_3']['parametros']['gamma']}, janela={metadados['sugestao_3']['parametros']['janela']}")
