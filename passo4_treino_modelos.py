...
============================================================
Módulo: passo4_treino_modelos.py
Autor: Pipeline Africa MO
Versão: 2.1 (com Rec. 9 e Rec. 14)

Descrição
----------
Treina 7 modelos preditivos de 4 paradigmas distintos sobre 4
datasets, utilizando validação walk-forward temporal com 5 folds.
Inclui diagnóstico bayesiano completo (R-hat, ESS, HDI, trace
plots) para os modelos MCMC.
...
Modelos (7):
  ...
  6. Bayesiano Hierárquico - Partial Pooling (PyMC/MCMC) [Rec. 9]
  7. Bayesiano Complete Pooling (PyMC/MCMC) [Rec. 9]
...
Diagnóstico Bayesiano [Rec. 9]:
  - R-hat (Gelman-Rubin): convergência das cadeias MCMC
  - ESS (Effective Sample Size): amostras independentes
  - HDI 94%: intervalos de maior densidade posterior
  - Trace plots: visualização das cadeias
  - Exportação automática para modelos/diagnostico_bayesiano/
...
- dados_engenharia/wdi_sintetico_features.csv

Saídas
------
- modelos/modelo_{Dataset}_{Modelo}.pkl (28 modelos)
- modelos/diagnostico_bayesiano/*.csv (R-hat, ESS, HDI) [Rec. 9]
- modelos/diagnostico_bayesiano/*.png (trace plots) [Rec. 9]
- modelos/metricas_walk_forward.csv
- modelos/metricas_split_final.csv
- modelos/*.png (6 gráficos)

Dependencias
------------
numpy, pandas, scikit-learn, xgboost, statsmodels, pmdarima,
tensorflow, pymc, arviz, scipy, pickle
============================================================
"""
import os
import sys
...
# ============================================================
# SUPRESSÃO TOTAL DE WARNINGS (statsmodels ConvergenceWarning)
# ============================================================
# O statsmodels no Python 3.12 ignora warnings.catch_warnings() locais
# porque usa warnings.warn() com stacklevel que bypassa filtros contextuais.
# Solução: filtro GLOBAL permanente por categoria + monkey-patch do módulo.
warnings.filterwarnings('ignore')
warnings.filterwarnings('ignore', category=FutureWarning)
warnings.filterwarnings('ignore', message='.*Maximum Likelihood.*')
warnings.filterwarnings('ignore', module='statsmodels.*')

# Importar e silenciar statsmodels ANTES de qualquer uso
try:
...
# ============================================================
# MODELO 4: SARIMAX COM EXÓGENAS (statsmodels)
# ============================================================

class SARIMAXExogPredictor:
    """
    SARIMAX real com variáveis exógenas.
    
    Estratégia picklable:
      - Guarda endog (y_train) e exog (X_train) de treino + parâmetros ajustados
      - No predict, reconstrói o modelo SARIMAX com os dados de treino,
        aplica os parâmetros (apply), e usa forecast(steps=n, exog=X_test)
      - Isto preserva a dinâmica AR/MA completa para previsão out-of-sample
      - Fallback para Ridge se SARIMAX falhar em qualquer etapa
    """
    def __init__(self, params, order, endog_train, exog_train,
                 top_features_idx, n_features_orig,
                 scaler, use_fallback=False, fallback_coef=None,
                 fallback_intercept=None):
        self.params = params              # Parâmetros SARIMAX ajustados
        self.order = order                # (p, d, q)
        self.endog_train = endog_train    # Série y de treino (para reconstruir)
        self.exog_train = exog_train      # Exógenas de treino (normalizadas)
        self.top_features_idx = top_features_idx
        self.n_features_orig = n_features_orig
        self.scaler = scaler
        self.use_fallback = use_fallback
        self.fallback_coef = fallback_coef
        self.fallback_intercept = fallback_intercept
        self.is_sarimax_real = not use_fallback
    
    def predict(self, X):
        """
        Previsão out-of-sample usando forecast real do SARIMAX.
        
        Reconstrói o modelo com dados de treino + parâmetros,
        depois usa forecast(steps=n, exog=X_test_scaled).
        """
        X_arr = np.array(X)
        
        # Seleccionar e normalizar features
        if X_arr.shape[1] > len(self.top_features_idx):
            X_sel = X_arr[:, self.top_features_idx]
        else:
            X_sel = X_arr
        X_s = self.scaler.transform(X_sel)
        
        if self.use_fallback:
            return X_s @ self.fallback_coef + self.fallback_intercept
        
        try:
            from statsmodels.tsa.statespace.sarimax import SARIMAX as SM_SARIMAX
            
            # Reconstruir modelo e forecast (warnings suprimidos)
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                model = SM_SARIMAX(
                    endog=self.endog_train,
                ...
                # Aplicar parâmetros já ajustados (sem re-estimar)
                result = model.smooth(self.params)
                
                # Forecast out-of-sample com exógenas do teste
                n_forecast = X_s.shape[0]
                forecast = result.forecast(steps=n_forecast, exog=X_s)
                forecast_arr = np.array(forecast)
            
            # Validar resultado
            if np.any(np.isnan(forecast_arr)) or np.any(np.isinf(forecast_arr)):
                raise ValueError("Forecast produziu NaN/Inf")
...
def treinar_sarimax(X_train, y_train, X_val, y_val):
    ...
    Ajusta SARIMAX(endog=y, exog=X_top, order=(1,1,1)) usando as top-K
    features mais correlacionadas com o target. Valida com forecast real
    no conjunto de validação. Guarda dados de treino + parâmetros para
    reconstrução no predict (picklable).
    
    Fallback: Ridge regression se SARIMAX falhar ou divergir.
    """
    ...
    # ================================================================
    # 1. Seleccionar top-K features (SARIMAX instável com muitas exógenas)
    # ================================================================
    MAX_EXOG = min(8, n_features)  # Máximo 8 exógenas para estabilidade
    
    # Correlação absoluta com target
    correlations = np.array([
        abs(np.corrcoef(X_arr[:, i], y_arr)[0, 1]) 
        if np.std(X_arr[:, i]) > 1e-10 else 0.0
        for i in range(n_features)
    ])
    top_features_idx = np.argsort(correlations)[-MAX_EXOG:]
    X_sel = X_arr[:, top_features_idx]
    
    # ================================================================
    # 2. Normalizar exógenas (SARIMAX sensível a escalas)
    # ================================================================
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_sel)
    
    # ================================================================
    # 3. Treinar Ridge como fallback (sempre disponível)
    # ================================================================
    ridge = Ridge(alpha=1.0)
    ridge.fit(X_scaled, y_arr)
    fallback_coef = ridge.coef_
    fallback_intercept = ridge.intercept_
    
    # ================================================================
    # 4. Tentar SARIMAX real com exógenas
    # ================================================================
    order = (1, 1, 1)
    use_fallback = True
    sarimax_params = None
    ...
    try:
        ...
        # SARIMAX precisa de observações suficientes
        min_obs = sum(order) + 10
        if n_samples >= min_obs:
            # Ajustar SARIMAX com exógenas (warnings suprimidos)
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                model = SM_SARIMAX(
                    endog=y_arr,
            ...
            # Validar no conjunto de validação com forecast REAL
            X_val_arr = np.array(X_val)
            X_val_sel = X_val_arr[:, top_features_idx]
            X_val_scaled = scaler.transform(X_val_sel)
            y_val_arr = np.array(y_val).ravel()
            
            # Forecast out-of-sample com exógenas (warnings suprimidos)
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                n_forecast = len(y_val_arr)
                forecast = result.forecast(steps=n_forecast, exog=X_val_scaled)
                forecast_arr = np.array(forecast)
            
            # Verificar que o forecast é razoável
            if (not np.any(np.isnan(forecast_arr)) and 
                not np.any(np.isinf(forecast_arr)) and
                np.std(forecast_arr) > 0):
                
                # Comparar RMSE: SARIMAX vs Ridge
                rmse_sarimax = np.sqrt(np.mean((y_val_arr - forecast_arr)**2))
                ridge_pred = X_val_scaled @ fallback_coef + fallback_intercept
                rmse_ridge = np.sqrt(np.mean((y_val_arr - ridge_pred)**2))
                
                # Usar SARIMAX se for melhor ou comparável ao Ridge
                if rmse_sarimax <= rmse_ridge * 2.0:
                    sarimax_params = np.array(result.params)
                    use_fallback = False
    
    except Exception:
        pass  # Fallback para Ridge
    
    # ================================================================
    # 5. Retornar predictor picklable
    # ================================================================
    return SARIMAXExogPredictor(
        ...
        fallback_intercept=fallback_intercept
    )
...
def treinar_bayesiano_partial(X_train, y_train, X_val, y_val):
    ...
    try:
        ...
        # Rec. 9: Exportar diagnósticos MCMC
        try:
            exportar_diagnostico_bayesiano(trace, 'Bayes_PartialPooling', 'current', config)
        except Exception:
            pass
        
        predictor = BayesPartialPredictor(alpha_post, beta_post, scaler, top_idx, y_mean)
        predictor._trace = trace  # Guardar trace para diagnósticos no passo 5/9
        return predictor
...
def exportar_diagnostico_bayesiano(trace, modelo_nome, ds_nome, config):
    """
    Exporta diagnósticos MCMC completos (Rec. 9).
    
    Gera:
      - CSV com R-hat, ESS, HDI por parâmetro
      - PNG com trace plots (cadeias)
      - PNG com distribuições posteriores
    
    Parameters
    ----------
    trace : arviz.InferenceData
        Trace retornado por pm.sample(return_inferencedata=True).
    modelo_nome : str
        Nome do modelo (ex: 'Bayes_PartialPooling').
    ds_nome : str
        Nome do dataset (ex: 'Agregado').
    config : module
        Módulo config_global.
    ...
    """
    ficheiros = []
    diag_dir = os.path.join(config.MODELOS_DIR, 'bayesian_diagnostics')
    ...
    try:
        ...
        prefix = f'{ds_nome}_{modelo_nome}'
        
        # 1. Sumário com R-hat, ESS, HDI
        summary = az.summary(trace, hdi_prob=0.94,
                           var_names=['alpha', 'beta', 'sigma'],
                           round_to=4)
        summary_path = os.path.join(diag_dir, f'{prefix}_summary.csv')
        summary.to_csv(summary_path)
        ficheiros.append(summary_path)
        
        # 2. R-hat check
        rhat_vals = summary['r_hat'].values if 'r_hat' in summary.columns else []
        rhat_ok = all(r < 1.05 for r in rhat_vals if not np.isnan(r))
        
        # 3. Trace plots (cadeias)
        try:
            fig = plt.figure(figsize=(12, 8))
            az.plot_trace(trace, var_names=['alpha', 'sigma'],
                         compact=True, combined=False)
            plt.suptitle(f'Trace Plot: {prefix}\nR-hat OK: {rhat_ok}',
                        fontsize=11, fontweight='bold')
            plt.tight_layout()
            trace_path = os.path.join(diag_dir, f'{prefix}_trace_plot.png')
            plt.savefig(trace_path, dpi=150, bbox_inches='tight')
            plt.close('all')
            ficheiros.append(trace_path)
        except Exception:
            pass
        
        # 4. Distribuições posteriores com HDI
        try:
            fig = plt.figure(figsize=(12, 6))
            az.plot_posterior(trace, var_names=['alpha', 'sigma'],
                           hdi_prob=0.94, point_estimate='mean')
            plt.suptitle(f'Posteriores: {prefix} (HDI 94%)',
                        fontsize=11, fontweight='bold')
            plt.tight_layout()
            post_path = os.path.join(diag_dir, f'{prefix}_posterior.png')
            plt.savefig(post_path, dpi=150, bbox_inches='tight')
            plt.close('all')
            ficheiros.append(post_path)
        except Exception:
            pass
        
        # 5. Energy plot (diagnóstico de convergência)
        try:
            fig, ax = plt.subplots(figsize=(8, 5))
            az.plot_energy(trace, ax=ax)
            ax.set_title(f'Energy Plot: {prefix}', fontweight='bold')
            plt.tight_layout()
            energy_path = os.path.join(diag_dir, f'{prefix}_energy.png')
            plt.savefig(energy_path, dpi=150, bbox_inches='tight')
            plt.close('all')
            ficheiros.append(energy_path)
        except Exception:
            pass
        ...
        diag_json = {
            ...
            'rhat_max': float(max(rhat_vals)) if len(rhat_vals) > 0 else None,
            ...
            'n_divergences': int(trace.sample_stats['diverging'].values.sum()) if hasattr(trace, 'sample_stats') else 0,
            'hdi_94_alpha': [float(summary.loc['alpha', 'hdi_3%']), float(summary.loc['alpha', 'hdi_97%'])] if 'alpha' in summary.index else None,
        }
        json_path = os.path.join(diag_dir, f'{prefix}_diagnostics.json')
        with open(json_path, 'w') as f:
            json.dump(diag_json, f, indent=2)
        ficheiros.append(json_path)
        
        print(f" Diag:R-hat={'OK' if rhat_ok else 'WARN'}", end='', flush=True)
        
    except (ImportError, Exception) as e:
        print(f" Diag:SKIP({str(e)[:20]})", end='', flush=True)
    
    return ficheiros


# ============================================================
# MODELO 7: BAYESIANO COMPLETE POOLING — CLASSES E FUNÇÃO
# ============================================================
...
def treinar_bayesiano_complete(X_train, y_train, X_val, y_val):
    ...
    try:
        ...
        # Rec. 9: Exportar diagnósticos MCMC
        try:
            exportar_diagnostico_bayesiano(trace, 'Bayes_CompletePooling', 'current', config)
        except Exception:
            pass
        
        predictor = BayesCompletePredictor(alpha_post, beta_post, scaler, pca, y_mean)
        predictor._trace = trace  # Guardar trace para diagnósticos no passo 5/9
        return predictor
...
def executar_passo4():
    ...
    for ds_nome, df in datasets.items():
        ...
        for modelo_nome, func_treino in modelos_config.items():
            t0 = time.time()
            # Flush stderr para evitar interleaving de warnings com stdout
            sys.stderr.flush()
            sys.stdout.flush()
            print(f"\n    [{modelo_nome}]", end="", flush=True)
            
            # --- Walk-forward (warnings suprimidos para output limpo) ---
