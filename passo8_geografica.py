...
- modelos/modelo_Agregado_*.pkl (RandomForest, XGBoost, GB)

Saidas
------
- analise_geografica/performance_por_pais.csv
- analise_geografica/convergencia_beta.csv
- analise_geografica/quebras_estruturais.csv
- analise_geografica/clusters_geograficos.csv
- analise_geografica/analise_regional.csv
- analise_geografica/*.png (7 graficos)
...
Contribuicao
-----------
- Artigo 1: Seccao 5 (heterogeneidade geografica)
- Artigo 2: Seccao 3 (convergencia beta, Chow)
- Dissertacao: Capitulo 5 (questao Q3)
============================================================
"""
...
def executar_passo8():
    ...
    if 'country_code' in df.columns and target in df.columns:
        for pais in df['country_code'].unique():
            ...
            wgi_cols = [c for c in df_p.columns if ('wgi_pca' in c.lower() or 'pca1' in c.lower()) and 'inter_' not in c.lower()]
            if wgi_cols:
