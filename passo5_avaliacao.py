...
Analises Completas:
  ...
  4. Teste Diebold-Mariano com bootstrap por cluster (correção para dados em painel)
 ...
 10. Exportacao de previsoes e residuos (CSV)
...
Recomendacoes Implementadas:
  ...
  - **Correção metodológica**: Teste DM com bootstrap por cluster para dados em painel
...
def diebold_mariano_test(e1, e2, h=1):
    """
    Teste Diebold-Mariano bilateral para comparacao de modelos (versao classica).
...
def diebold_mariano_cluster_bootstrap(e1, e2, cluster_ids, n_boot=1000):
    """
    Teste de Diebold-Mariano com bootstrap por cluster (paises) - CORRECAO PARA PAINEL.

    Parameters
    ----------
    e1 : array-like
        Erros do modelo 1 (referencia).
    e2 : array-like
        Erros do modelo 2 (alternativo).
    cluster_ids : array-like
        Identificador do cluster (pais) para cada observacao.
    n_boot : int, optional
        Numero de replicacoes bootstrap (default=1000).

    Returns
    -------
    dict
        Contem:
        - 'dm_stat': estatistica DM observada (d_mean / se_boot)
        - 'p_value': p-valor bilateral (bootstrap)
        - 'ci_lower', 'ci_upper': intervalo de confianca bootstrap a 95% para d_mean
        - 'd_mean': diferenca media observada
        - 'se_boot': erro padrao bootstrap de d_mean
    ...
    e1, e2, cluster_ids = e1[mask], e2[mask], cluster_ids[mask]
    
    # Calcular perdas quadraticas
    d = e1**2 - e2**2
    d_mean_obs = np.mean(d)
    
    # Obter clusters unicos
    clusters_unicos = np.unique(cluster_ids)
    n_clusters = len(clusters_unicos)
    
    # Se houver poucos clusters, usar metodo classico (fallback)
    if n_clusters < 3:
        ...
        return {
            ...
            'd_mean': d_mean_obs,
            'se_boot': np.nan,
            'metodo': 'Newey-West (classico)'
        }
    
    # Armazenar as series de d para cada cluster
    d_por_cluster = {c: d[cluster_ids == c] for c in clusters_unicos}
    
    # Bootstrap por cluster
    d_boot = []
    for _ in range(n_boot):
        # Reamostrar clusters com reposicao
        idx_boot = np.random.choice(n_clusters, size=n_clusters, replace=True)
        # Concatenar as series de d dos clusters selecionados
        d_boot_sample = np.concatenate([d_por_cluster[clusters_unicos[i]] for i in idx_boot])
        d_boot.append(np.mean(d_boot_sample))
    
    d_boot = np.array(d_boot)
    se_boot = np.std(d_boot, ddof=1)
    
    # Estatistica DM (usando erro padrao bootstrap)
    dm_stat = d_mean_obs / (se_boot + 1e-10)
    
    # P-valor bilateral (proporcao de |d_boot| > |d_mean_obs|)
    p_value = 2 * min(
        np.mean(d_boot > d_mean_obs),
        np.mean(d_boot < d_mean_obs)
    )
    p_value = min(p_value, 1.0)  # truncar
    
    # Intervalo de confianca bootstrap (percentil)
    ...
    return {
        ...
        'd_mean': d_mean_obs,
        'se_boot': se_boot,
        'metodo': f'Bootstrap por cluster (n={n_clusters} paises)'
    }
...
def executar_passo5():
    ...
    Executa:
      ...
      3. Teste Diebold-Mariano com bootstrap por cluster (correcao para painel)
      ...
      8. Exportacao de previsoes e residuos (CSV)
    ...
    print("  (COM CORRECAO PARA DADOS EM PAINEL - DM BOOTSTRAP POR CLUSTER)")
    print("=" * 70)
    ...
    # ============================================================
    # [1/9] CARREGAR E AVALIAR MODELOS (com cluster_ids e anos)
    # ============================================================
    ...
    for ds_nome, ds_path in ds_paths.items():
        ...
        y = df[target].values
        country_codes = df['country_code'].values
        ...
        country_test = country_codes[test_start:]
        years_test = years[test_start:]
        ...
        for modelo_nome in modelos_nomes:
            ...
            try:
                ...
                # Guardar previsoes com cluster_ids e anos
                previsoes[(ds_nome, modelo_nome)] = {
                    'y_true': y_test,
                    'y_pred': y_pred,
                    'erros': erros,
                    'cluster_ids': country_test,
                    'years': years_test
                }
    ...
    # ============================================================
    # [4/9] TESTE DIEBOLD-MARIANO COM BOOTSTRAP POR CLUSTER (CORRECAO PAINEL)
    # ============================================================
    print("\n  [4/9] Teste Diebold-Mariano (WDI_Limpo vs Agregado) com bootstrap por cluster...")
    ...
    for modelo_nome in modelos_nomes:
        ...
        if key_wdi in previsoes and key_agg in previsoes:
            dados_wdi = previsoes[key_wdi]
            dados_agg = previsoes[key_agg]
            e_wdi = dados_wdi['erros']
            e_agg = dados_agg['erros']
            cluster_ids = dados_wdi['cluster_ids']  # pais devem ser os mesmos
            
            # Garantir mesmo comprimento
            min_len = min(len(e_wdi), len(e_agg), len(cluster_ids))
            e_wdi = e_wdi[:min_len]
            e_agg = e_agg[:min_len]
            cluster_ids = cluster_ids[:min_len]
            
            rmse_wdi = np.sqrt(np.mean(e_wdi**2))
            rmse_agg = np.sqrt(np.mean(e_agg**2))
            ganho_pct = (rmse_wdi - rmse_agg) / (rmse_wdi + 1e-10) * 100
            
            if len(e_wdi) >= 5:
                # Usar bootstrap por cluster
                dm_res = diebold_mariano_cluster_bootstrap(e_wdi, e_agg, cluster_ids, n_boot=1000)
                dm_stat = dm_res['dm_stat']
                p_value = dm_res['p_value']
                ci_lower = dm_res['ci_lower']
                ci_upper = dm_res['ci_upper']
                metodo = dm_res['metodo']
            else:
                dm_stat, p_value = np.nan, np.nan
                ci_lower, ci_upper = np.nan, np.nan
                metodo = 'N/A (amostra insuficiente)'
            ...
            dm_resultados.append({
                ...
                'P_Value': p_value,
                'DM_CI_Lower': ci_lower,
                'DM_CI_Upper': ci_upper,
                'Metodo': metodo,
                ...
                'Interpretacao': 'Agregado significativamente melhor' if (not np.isnan(p_value) and p_value < 0.05 and ganho_pct > 0) else ('Agregado significativamente pior' if (not np.isnan(p_value) and p_value < 0.05 and ganho_pct < 0) else 'Diferenca nao significativa')
            })
            print(f"    {modelo_nome}: DM={dm_stat:.3f}, p={p_value:.4f}, Ganho={ganho_pct:.1f}% (metodo: {metodo})")
    ...
    # ============================================================
    # [9/9] EXPORTACAO DE PREVISOES E RESIDUOS (CSV)
    # ============================================================
    print("\n  [9/9] Exportando dataset de previsoes e residuos (para analise detalhada)...")
    rows_residuos = []
    for (ds_nome, modelo_nome), dados in previsoes.items():
        y_true = dados['y_true']
        y_pred = dados['y_pred']
        erros = dados['erros']
        cluster_ids = dados['cluster_ids']
        years = dados['years']
        n = len(y_true)
        for i in range(n):
            rows_residuos.append({
                'Dataset': ds_nome,
                'Modelo': modelo_nome,
                'country_code': cluster_ids[i] if i < len(cluster_ids) else np.nan,
                'year': years[i] if i < len(years) else np.nan,
                'y_true': y_true[i],
                'y_pred': y_pred[i],
                'residuo': erros[i]
            })
    df_residuos = pd.DataFrame(rows_residuos)
    residuos_path = os.path.join(config.RESULTADOS_DIR, 'previsoes_residuos.csv')
    df_residuos.to_csv(residuos_path, index=False)
    print(f"    Dataset de residuos salvo em {residuos_path} ({len(df_residuos)} linhas)")
    
    # ============================================================
    # [10/9] METADADOS E RESUMO
    # ============================================================
    ficheiros_saida = [avaliacao_path, resumo_path, resumo_ds_path, dm_path,
                       ic_path, ganho_path, ranking_path, baselines_path, residuos_path]
    
    gerar_metadados(
        passo='passo5_avaliacao',
        descricao='Avaliacao completa: 8 metricas, baselines ingenuas, DM bootstrap por cluster, IC 95%, ranking composto, residuos',
        config=config,
        ...
        parametros={'metricas': metricas_cols + ['AIC'], 'bootstrap_n': 1000,
                    ...
                    'dm_method': 'Bootstrap por cluster (correcao para painel)'},
        metricas={
