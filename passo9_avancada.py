...
def executar_passo9():
    print("\n" + "=" * 70)
    print("  PASSO 9: ANALISES AVANCADAS (CORRIGIDO)")
    print("=" * 70)
    os.makedirs(config.AVANCADA_DIR, exist_ok=True)
    t_inicio = time.time()

    # ============================================================
    # 1. CARREGAR DADOS E MODELOS (MAIS ROBUSTO)
    # ============================================================
    ...
    df_agg = datasets.get('Agregado', next(iter(datasets.values())) if datasets else None)
    if df_agg is None:
        print("  ERRO: Nenhum dataset encontrado.")
        return

    # Carregar qualquer modelo que tenha predict (prioridade para RandomForest do Agregado)
    best_model = None
    model_paths = []
    for modelo_nome in ['RandomForest', 'XGBoost', 'GradientBoosting']:
        mp = os.path.join(config.MODELOS_DIR, f'modelo_Agregado_{modelo_nome}.pkl')
        if os.path.exists(mp):
            model_paths.append(mp)
    if not model_paths:
        # Tenta qualquer modelo .pkl que contenha 'Agregado' no nome
        for f in os.listdir(config.MODELOS_DIR):
            if f.endswith('.pkl') and 'Agregado' in f:
                model_paths.append(os.path.join(config.MODELOS_DIR, f))
    for mp in model_paths:
        try:
            ...
            if hasattr(candidate, 'predict'):
                ...
                print(f"    Modelo carregado: {os.path.basename(mp)}")
                break
        except Exception:
            continue

    if best_model is None:
        print("    AVISO: Nenhum modelo com .predict() encontrado. Algumas análises serão ignoradas.")
    else:
        print(f"    Modelo OK: {type(best_model).__name__}")

    # ============================================================
    # 2. IDENTIFICAR VARIÁVEIS WGI (ORIGINAIS + PCA)
    # ============================================================
    # Procura colunas que começam com 'wgi_' (ex: wgi_controle_corrupcao) 
    # OU que contenham 'pca' (ex: pca1, wgi_pca1) – exclui interações
    wgi_cols = [c for c in df_agg.columns 
                if (c.startswith('wgi_') or 'pca' in c.lower()) 
                and 'inter_' not in c.lower()
                and c != target]
    if not wgi_cols:
        print("    AVISO: Nenhuma variável WGI identificada. Análises de sensibilidade/elasticidade ignoradas.")
    else:
        print(f"    {len(wgi_cols)} variáveis WGI encontradas: {wgi_cols[:3]}...")

    # ============================================================
    # 3. CONVERGÊNCIA SIGMA (funcionou)
    # ============================================================
    ...
    if 'country_code' in df_agg.columns and 'year' in df_agg.columns and target in df_agg.columns:
        for year in sorted(df_agg['year'].unique()):
            ...
            if len(vals) >= 3:
                sigma_data.append({'Ano': year, 'Std': vals.std(), 'CV': vals.std() / (vals.mean() + 1e-10), 
                                   'Media': vals.mean(), 'N': len(vals), 'Min': vals.min(), 'Max': vals.max(), 
    ...
    # ============================================================
    # 4. ANÁLISE DE SENSIBILIDADE (TORNADO) – com verificação
    # ============================================================
    print("\n  [3/8] Analise de sensibilidade (tornado)...")
    sensibilidade = []
    if best_model is not None and target in df_agg.columns and len(wgi_cols) > 0:
        ...
        for col in wgi_cols:
            ...
            sensibilidade.append({
                'Variavel': col,
                'Impacto_Up': y_up - y_base_pred,
                'Impacto_Down': y_down - y_base_pred,
                'Impacto_Abs': abs(y_up - y_base_pred) + abs(y_down - y_base_pred),
                'IC_Up_Lower': np.percentile(deltas_up, 2.5),
                'IC_Up_Upper': np.percentile(deltas_up, 97.5),
                'IC_Down_Lower': np.percentile(deltas_down, 2.5),
                'IC_Down_Upper': np.percentile(deltas_down, 97.5),
    ...
    if sensibilidade:
        ...
        print("    Nenhum dado de sensibilidade gerado (modelo ou WGI ausentes).")

    # ============================================================
    # 5. CENÁRIOS DE POLÍTICA (com verificação)
    # ============================================================
    ...
    if best_model is not None and target in df_agg.columns and len(wgi_cols) > 0:
        ...
        for cenario_nome, multiplicador in [('Pessimista', -1.0), ('Base', 0.0), ('Otimista', 1.0)]:
            ...
            cenarios.append({'Cenario': cenario_nome, 'Previsao_Media': y_cen, 
                             'Delta_vs_Base': y_cen - y_base_mean, 
                             'Delta_Pct': (y_cen - y_base_mean) / (abs(y_base_mean) + 1e-10) * 100})
        for col in wgi_cols[:6]:
            ...
            for mult, nome in [(-1, 'Pessimista'), (1, 'Otimista')]:
                ...
                cenarios.append({'Cenario': f'{nome}_{col}', 'Previsao_Media': y_c, 
                                 'Delta_vs_Base': y_c - y_base_mean, 
                                 'Delta_Pct': (y_c - y_base_mean) / (abs(y_base_mean) + 1e-10) * 100})
    ...
    # ============================================================
    # 6. ELASTICIDADES (com verificação)
    # ============================================================
    print("\n  [5/8] Elasticidades...")
    elasticidades = []
    if best_model is not None and target in df_agg.columns and len(wgi_cols) > 0:
    ...
    if not df_elast.empty and 'Elasticidade' in df_elast.columns:
    ...
    # ============================================================
    # 7. CAUSALIDADE DE GRANGER
    # ============================================================
    print("\n  [6/8] Causalidade de Granger...")
    granger_results = []
    if 'country_code' in df_agg.columns and 'year' in df_agg.columns and target in df_agg.columns and len(wgi_cols) > 0:
        try:
            ...
            for pais in df_agg['country_code'].unique()[:15]:
                ...
                for col in wgi_cols:
                    ...
                    try:
                        ...
                        granger_results.append({'Pais': pais, 'WGI': col, 'F_Stat': f_stat, 
                                                'P_Value': p_val, 'Significativo': 'Sim' if p_val < 0.05 else 'Nao', 'Lag': 1})
                    except Exception:
                        pass
        except ImportError:
            print("    AVISO: statsmodels não disponível para Granger")
    ...
    n_sig_granger = len(df_granger[df_granger['Significativo'] == 'Sim']) if not df_granger.empty else 0
    print(f"    Granger significativos: {n_sig_granger}/{len(df_granger)}")

    # ============================================================
    # 8. ROBUSTEZ (missing data) – já com verificações
    # ============================================================
    print("\n  [7/8] Analise de robustez...")
    robustez = []
    if best_model is not None and target in df_agg.columns:
        ...
        robustez.append({'Missing_Pct': 0, 'RMSE': rmse_full, 'R2': r2_full, 
                         'RMSE_Degradacao_Pct': 0, 'R2_Degradacao_Pct': 0, 'RMSE_Std': 0, 'R2_Std': 0})
    ...
    # ============================================================
    # 9. VISUALIZAÇÕES (com verificações de segurança)
    # ============================================================
    print("\n  [8/8] Gerando visualizações...")

    # Gráfico 1: Convergência sigma
    if len(df_sigma) > 3:
    ...
    # Gráfico 2: Tornado plot (se existir df_sens não vazio)
    ...
    # Gráfico 3: Cenários de política
    if not df_cenarios.empty:
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        ax = axes[0]
        cen_global = df_cenarios[df_cenarios['Cenario'].isin(['Pessimista', 'Base', 'Otimista'])]
        if not cen_global.empty:
            colors_c = {'Pessimista': 'red', 'Base': 'gray', 'Otimista': 'green'}
            ax.bar(range(len(cen_global)), cen_global['Previsao_Media'], 
                   color=[colors_c.get(c, 'blue') for c in cen_global['Cenario']], alpha=0.7)
        ...
        if not cen_var.empty:
            ax.barh(range(len(cen_var)), cen_var['Delta_Pct'], 
                    color=['green' if d > 0 else 'red' for d in cen_var['Delta_Pct']], alpha=0.7)
    ...
    if len(df_robustez) > 1:
        ...
        ax.fill_between(df_robustez['Missing_Pct'], df_robustez['RMSE'] - df_robustez['RMSE_Std'], 
                        df_robustez['RMSE'] + df_robustez['RMSE_Std'], alpha=0.2, color='red')
        ...
        ax.fill_between(df_robustez['Missing_Pct'], df_robustez['R2'] - df_robustez['R2_Std'], 
                        df_robustez['R2'] + df_robustez['R2_Std'], alpha=0.2, color='blue')
        ax.set_xlabel('% Dados Faltantes')
        ax.set_ylabel('R2')
    ...
    # Gráfico 7: Projeções futuras (sintético)
    df_sint = datasets.get('Sintetico_Agregado')
    if df_sint is not None and best_model is not None and target in df_sint.columns and 'year' in df_sint.columns:
        ...
        try:
            ...
            ax.fill_between(proj['year'], proj['mean'] - proj['std'], proj['mean'] + proj['std'], 
                            alpha=0.2, color='blue', label='IC (1 Std)')
    ...
    # ============================================================
    # 10. INCERTEZA BAYESIANA (Rec. 11) – mantido original, mas com verificações
    # ============================================================
    ...
    try:
        ...
        if bayes_model and hasattr(bayes_model, '_trace'):
            ...
            # Gráfico 8: Incerteza Bayesiana
            fig, axes = plt.subplots(2, 2, figsize=(16, 12))
            # 8a: Previsão vs Real
            ax = axes[0, 0]
            if 'Year' in df_bayes_unc.columns:
                by_year = df_bayes_unc.groupby('Year').agg({'Pred_Mean': 'mean', 'HDI_3%': 'mean', 'HDI_97%': 'mean', 'Real': 'mean'}).reset_index()
                ...
                ax.fill_between(by_year['Year'], by_year['HDI_3%'], by_year['HDI_97%'], alpha=0.25, color='blue', label='HDI 94%')
                ax.set_xlabel('Ano')
            else:
                ax.plot(df_bayes_unc['Real'], 'ko-', markersize=3, label='Real')
                ax.plot(df_bayes_unc['Pred_Mean'], 'b-', linewidth=1.5, label='Posterior Mean')
                ax.fill_between(range(len(df_bayes_unc)), df_bayes_unc['HDI_3%'], df_bayes_unc['HDI_97%'], alpha=0.25, color='blue', label='HDI 94%')
                ax.set_xlabel('Observação')
            ax.set_ylabel(target)
            ax.set_title(f'Previsão Bayesiana com Incerteza ({bayes_nome})', fontweight='bold')
            ax.legend()
            ax.grid(alpha=0.3)
            # 8b: Distribuição da largura HDI
            ax = axes[0, 1]
            ...
            # 8c: Calibração
            ax = axes[1, 0]
            dentro_hdi = ((df_bayes_unc['Real'] >= df_bayes_unc['HDI_3%']) & (df_bayes_unc['Real'] <= df_bayes_unc['HDI_97%'])).mean() * 100
            ...
            ax.legend()
            ax.grid(alpha=0.3)
            # 8d: Incerteza por país
            ax = axes[1, 1]
            if 'Country' in df_bayes_unc.columns:
                unc_by_country = df_bayes_unc.groupby('Country')['Pred_Std'].mean().sort_values()
                ...
                ax.barh(range(len(combined)), combined.values, color=colors_unc, alpha=0.7)
        ...
        elif bayes_model and hasattr(bayes_model, 'predict'):
            # Fallback BayesianRidge
            X_pred = df_agg.drop(columns=['country_code', 'year', target, 'pais'], errors='ignore').fillna(0)
            try:
                y_pred_mean, y_pred_std = bayes_model.predict(X_pred, return_std=True)
                df_bayes_unc = pd.DataFrame({
                    'Pred_Mean': y_pred_mean, 'Pred_Std': y_pred_std,
                ...
                bayes_uncertainty_path = os.path.join(config.AVANCADA_DIR, 'incerteza_bayesiana.csv')
                df_bayes_unc.to_csv(bayes_uncertainty_path, index=False)
                print("    Fallback BayesianRidge: incerteza analítica exportada")
            except Exception:
                print("    Fallback: modelo não suporta return_std")
        else:
            print("    Nenhum modelo bayesiano com trace encontrado")
    except Exception as e:
        print(f"    Incerteza bayesiana: erro - {str(e)[:60]}")

    # ============================================================
    # METADADOS E FINALIZAÇÃO
    # ============================================================
    ficheiros_saida = [sigma_path, cenarios_path, elast_path, granger_path, robustez_path]
    if os.path.exists(sens_path):
        ficheiros_saida.append(sens_path)
    if bayes_uncertainty_path:
        ficheiros_saida.append(bayes_uncertainty_path)
    gerar_metadados(passo='passo9_avancada_corrigido',
                    descricao='Convergencia sigma, tornado com IC, cenarios, elasticidades bootstrap, Granger, robustez, incerteza bayesiana',
                    config=config, dados_entrada=list(datasets.keys()), dados_saida=ficheiros_saida,
                    parametros={'n_bootstrap_elast': 50, 'n_bootstrap_sens': 50, 'n_robustez_runs': 5, 'granger_maxlag': 2},
                    metricas={'sigma_convergencia': sigma_trend.get('convergencia', 'NA') if sigma_trend else 'NA',
                              'granger_significativos': n_sig_granger,
                              'n_elasticidades_sig': len(df_elast[df_elast['Significativo'] == 'Sim']) if not df_elast.empty else 0})
    auto_save_drive(ficheiros_saida, config)
    t_total = time.time() - t_inicio
    print(f"\n  RESUMO PASSO 9: sigma={'Conv' if sigma_trend and sigma_trend['convergencia']=='Sim' else 'Div'} | {n_sig_granger} Granger sig | {len([f for f in ficheiros_saida if f.endswith('.png')])} graficos | {t_total:.1f}s")
