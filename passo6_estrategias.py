...
def executar_passo6():
    ...
    wgi_cols = [c for c in df.columns if 'wgi' in c.lower() and 'lag' not in c and 'ma' not in c and 'delta' not in c]
    wdi_cols = [c for c in df.columns if c not in wgi_cols and c not in ['country_code', 'year', 'pais', target]
    ...
    if 'country_code' in df.columns and len(wgi_cols) > 0:
        wgi_pais = df.groupby('country_code')[wgi_cols].mean().dropna()
        ...
        if len(wgi_pais) >= 3:
            ...
            # Ordenar clusters por media WGI
            cluster_means = wgi_pais.groupby('Cluster')[wgi_cols[0]].mean().sort_values()
            label_map = {}
