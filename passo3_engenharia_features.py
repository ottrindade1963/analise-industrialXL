...
Entradas
--------
- dados_limpos/wdi_limpo.csv (WDI: 37 países, 19 indicadores)
- dados_limpos/wgi_limpo.csv (WGI: 37 países, 6 indicadores)
- dados_limpos/agregado.csv (INNER JOIN WDI+WGI)
- dados_limpos/sintetico_agregado.csv (500 anos sintéticos)

Saídas
------
- dados_engenharia/agregado_features.csv (dataset principal)
- dados_engenharia/wdi_limpo_features.csv (sem WGI)
- dados_engenharia/sintetico_agregado_features.csv
- dados_engenharia/wdi_sintetico_features.csv
- dados_engenharia/agregado_pca_loadings.csv
- dados_engenharia/pca_models/*.pkl (scaler + PCA)
- eda_engenharia/*.png (5 gráficos diagnósticos)

Dependencias
------------
numpy, pandas, scikit-learn (PCA, StandardScaler),
matplotlib, seaborn, joblib

Contribuição
-----------
- Artigo 1: Secção 2 (PCA out-of-sample, antecedência temporal)
- Artigo 2: Secção 2 (interações PCA×WDI)
- Dissertação: Capítulo 4 (engenharia de features)
============================================================
"""
import os
import sys
...
def engenharia_features(df, nome_dataset, viz_dir=None, pca_dir=None):
    ...
    # ============================================================
    # REMOVER VARIÁVEIS CONTEMPORÂNEAS (ANTECEDÊNCIA TEMPORAL)
    # ============================================================
    # Justificação científica: Para garantir previsão genuína (X_{t-1} → Y_t),
    # as variáveis económicas contemporâneas (mesmo período t do target) são
    # removidas. Mantemos APENAS os lags (t-1) que respeitam a causalidade
    # temporal e estariam disponíveis no momento real da previsão.
    # Excepções:
    #   - target_lag1/lag2: preditores autorregressivos legítimos
    #   - wgi_pca1: já removido acima (substituído por lags do PCA)
    #   - Variáveis derivadas (delta, log_ret, ma): baseadas em dados passados
    # Identificar variáveis derivadas (que já respeitam antecedência)
    sufixos_derivados = ('_lag1', '_lag2', '_lag3', '_ma3', '_delta', '_log_ret')
    prefixos_derivados = ('inter_',)
    cols_id_target = {'country_code', 'year', 'pais', config.TARGET_VAR}
    
    # Variáveis contemporâneas = quantitativas originais que NÃO são:
    #   - identificadores (country_code, year, pais)
    #   - target
    #   - derivadas (lags, deltas, log_ret, ma, interações)
    #   - PCA (wgi_pca1 será tratado separadamente)
    cols_contemp_remover = []
    for c in df.columns:
        # Ignorar identificadores e target
        if c in cols_id_target:
            continue
        # Ignorar variáveis derivadas (já baseadas em dados passados)
        if any(c.endswith(s) for s in sufixos_derivados):
            continue
        if any(c.startswith(p) for p in prefixos_derivados):
            continue
        # Ignorar PCA e suas derivadas
        if 'pca' in c:
            continue
        # Ignorar target lags
        if c.startswith(config.TARGET_VAR):
            continue
        # Tudo o resto é contemporâneo → remover
        cols_contemp_remover.append(c)
    
    if cols_contemp_remover:
        df = df.drop(columns=cols_contemp_remover)
        print(f"  [ANTECEDÊNCIA] Removidas {len(cols_contemp_remover)} variáveis "
              f"contemporâneas (mantidos apenas lags e derivadas)")
        print(f"    Removidas: {cols_contemp_remover}")
    else:
        print(f"  [ANTECEDÊNCIA] Nenhuma variável contemporânea a remover")
    
    # Remover wgi_pca1 contemporâneo (manter apenas lag1, lag2, ma3, delta)
    # O PCA contemporâneo (t) não estaria disponível no momento da previsão.
    # Nota: wgi_pca1_ma3 e wgi_pca1_delta usam dados até t (inclusive),
    #       mas como são médias móveis e diferenças, a informação dominante
    #       vem de t-1 e t-2. Mantemos por serem derivadas parciais.
    if 'wgi_pca1' in df.columns and 'wgi_pca1_lag1' in df.columns:
        df = df.drop(columns=['wgi_pca1'])
        print(f"  [ANTECEDÊNCIA] Removido wgi_pca1 contemporâneo (mantidos lag1, lag2, ma3, delta)")
